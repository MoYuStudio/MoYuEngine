...
class Assets:
    ...
    def set(self):
        for sheet_num in range(len(self.config['sheet_dictionary'])):
            for data_num in range(len(self.config['data_dictionary'])):
                if self.config['file_type'] == 'png':
                    self.config['data_dictionary'][data_num]['assets'] = pygame.image.load(self.config['path']+'\\'+str(self.config['sheet_dictionary'][sheet_num])+'\\'+str(self.config['data_dictionary'][data_num]['name'])+'.png').convert_alpha()
